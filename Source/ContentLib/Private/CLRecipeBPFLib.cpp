...
void UCLRecipeBPFLib::InitRecipeFromStruct(UContentLibSubsystem* Subsystem ,FContentLib_Recipe RecipeStruct, TSubclassOf<class UFGRecipe> Recipe,bool ClearIngredients,bool ClearProducts,bool ClearBuilders)
{
	...
	if (RecipeStruct.Category != "") {
		if (const TSubclassOf<UFGItemCategory> Out = UBPFContentLib::SetCategoryWithLoad(RecipeStruct.Category, Subsystem, false)) {
	...
	UBPFContentLib::AddToItemAmountArray(CDO->mIngredients, RecipeStruct.Ingredients, Subsystem->mItems, ClearIngredients);
	UBPFContentLib::AddToItemAmountArray(CDO->mProduct, RecipeStruct.Products, Subsystem->mItems, ClearProducts);
...
void UCLRecipeBPFLib::AddToSchematicUnlock(const TSubclassOf<class UFGRecipe> Recipe, FContentLib_Recipe RecipeStruct, UContentLibSubsystem* Subsystem)
{
	...
	for (const FString& SchematicToFind : RecipeStruct.UnlockedBy) {
		if (UClass* SchematicClass = UBPFContentLib::FindClassWithLog(SchematicToFind, UFGSchematic::StaticClass(), Subsystem)) {
...
{
	const auto CDO = Recipe.GetDefaultObject();
	if (!IsValid(CDO)) {
		return "";
	}
